...
OIDN_NAMESPACE_BEGIN
  ...
  template<typename T>
  struct TensorAccessor1D WHERE(T, IArithmetic)
  {
    TensorByteOffset1D<T, TensorLayout::x> getByteOffset;
    ...
    __subscript(int x) -> T
    {
      get { return *reinterpret<T*>(ptr + getByteOffset(x)); }
      set { *reinterpret<T*>(ptr + getByteOffset(x)) = newValue; }
    }
#endif
  };

  template<typename T, TensorLayout layout>
  struct TensorAccessor3D WHERE(T, IArithmetic)
  {
    TensorByteOffset3D<T, layout> getByteOffset;
    oidn_global char* ptr;
...
#else
    __subscript(int c, int h, int w) -> T
    {
      get { return *reinterpret<T*>(ptr + getByteOffset(c, h, w)); }
      set { *reinterpret<T*>(ptr + getByteOffset(c, h, w)) = newValue; }
    }

    oidn_host_device_inline vec3<T> get3(int c, int h, int w)
    {
      return vec3<T>(this[c,   h, w],
                     this[c+1, h, w],
                     this[c+2, h, w]);
    ...
    [mutating]
    oidn_host_device_inline void set3(int c, int h, int w, vec3<T> value)
    {
      this[c,   h, w] = value.x;
      this[c+1, h, w] = value.y;
      this[c+2, h, w] = value.z;
  ...
  template<typename T, TensorLayout layout>
  struct TensorAccessor4D WHERE(T, IArithmetic)
  {
    TensorByteOffset4D<T, layout> getByteOffset;
    oidn_global char* ptr;
...
#else
    __subscript(int o, int i, int h, int w) -> T
    {
      get { return *reinterpret<T*>(ptr + getByteOffset(o, i, h, w)); }
      set { *reinterpret<T*>(ptr + getByteOffset(o, i, h, w)) = newValue; }
    }
#endif
  };
